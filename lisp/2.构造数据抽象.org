...
6. (2.29) 
   ...
   2. total-weight
      #+begin_src elisp
		;; 遍历整个tree,重量相加返回
		(defun total-weight (tree)
		    (cond ((null tree) 0)
			    ((numberp tree) tree)
			    (t (+ (total-weight (car tree))
				(total-weight (cdr tree))))))
		(total-weight (list 1 2 (list 3 4) (list (list 5 6))))

		;; 只计算weitght,left代表的是length
		(defun total-weight (tree)
		    (cond ((null tree) 0)
			    ((numberp (cdr tree)) (cdr tree))
			    (t (+ (total-weight (car tree))
				(total-weight (cdr tree))))))
		(total-weight (list 3 4))

		(defun right-side-weight (tree)
		    (cond ((null tree) 0)  ;; 如果树为空，返回0
			    ((numberp tree) tree)  ;; 如果树是一个数字，返回数字
			    ((listp tree) (+ (right-side-weight (car (cdr tree)))  ;; 递归地计算右子树的权重
					    (if (and (numberp (car tree)) (null (cdr tree))) (car tree) 0)))  ;; 如果当前节点是一个数字并且没有右子树，添加当前节点的值
			    (t 0)))  ;; 其他情况，返回0

	;; 示例调用
	(right-side-weight (list (list 3 1) (list 2 9)))

	(defun right-side-weight (tree)
  (cond ((null tree) 0)  ;; 如果树为空，返回0
        ((numberp tree) tree)  ;; 如果树是一个数字，返回该数字
        ((listp tree) (+ (right-side-weight (car (cdr tree)))  ;; 递归计算右子树的权重
                         (right-side-weight (cdr (cdr tree)))))  ;; 递归计算右子树的右子树
        (t 0)))  ;; 其他情况，返回0

;; 示例调用
(right-side-weight (list (list 3 1) (list 2 9)))  ;; 结果应该是10
